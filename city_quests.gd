...
func get_world_map_bbcode() -> String:
	...
	t += "• [color=#ffd700]Король[/color] — [b]внутри донжона[/b] на троне (западный проход в стену двора, затем войти в зал замка). E — цепочка заданий: враги → жители → рычаги.\n"
	...
	t += "• [color=#e6c35c]Замок[/color] — восточнее арены: [b]войти можно[/b] через проход в западной стене, далее донжон с проходом к трону; во дворе — три рычага для квеста короля и бонусные рычаги.\n"
	...
	t += "[b]Король (замок, трон)[/b]\n"
	t += KingQuests.get_map_bbcode()
	if final_boss_spawned:
